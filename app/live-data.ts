...
function cityDistance(city: string) {
  const distances: Record<string, number> = {
    Dallas: 8, "Fort Worth": 31, Arlington: 22, Irving: 13, "Grand Prairie": 16,
    Denton: 39, Plano: 20, Richardson: 15, Carrollton: 19, "The Colony": 29,
    Frisco: 30, McKinney: 33, Lewisville: 28, Grapevine: 24, Bedford: 26,
    Burleson: 42, Mansfield: 34, Garland: 18, "North Richland Hills": 30,
  };
  return distances[city] ?? 25;
}
...
function calendarNodeToShow(node: any, source: CalendarSource): Show | null {
  const rawStart = typeof node?.startDate === "string" ? node.startDate : "";
  ...
  const distanceMiles = cityDistance(city);
...
async function collectCalendarSource(source: CalendarSource) {
  ...
  const html = await response.text();
  const nodes = flattenJsonLd(extractJsonLd(html)).filter(isEvent);
  return nodes.map((node) => calendarNodeToShow(node, source)).filter((show): show is Show => Boolean(show));
}

export async function fetchCommunityCalendarShows() {
  const settled = await Promise.allSettled(calendarSources.map(collectCalendarSource));
  const shows: Show[] = [];
  const warnings: string[] = [];
  settled.forEach((result, index) => {
    if (result.status === "fulfilled") shows.push(...result.value);
...
export async function getLiveShows(): Promise<LiveShowsResponse> {
  ...
  const [tmResult, venueResult, calendarResult] = await Promise.allSettled([
    fetchTicketmasterShows({ radiusMiles: 65, days: 30 }),
    fetchVenuePageShows(),
    fetchCommunityCalendarShows(),
  ]);
  ...
  } else warnings.push("Community-calendar collector failed.");

  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const horizon = new Date(today.getTime() + 45 * 86400000);
  const upcoming = [...direct, ...community, ...ticketmaster].filter((show) => {
  ...
  const mode: LiveShowsResponse["mode"] = live.length ? (hasKey && (direct.length || community.length) ? "live" : "hybrid") : "demo";
