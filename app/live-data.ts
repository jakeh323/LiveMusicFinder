...
export async function fetchTicketmasterShows(options: {
  ...
  return allEvents.flatMap((event): Show[] => {
...
export async function fetchJamBaseShows(options: { origin: SearchOrigin; radiusMiles: number; days?: number }) {
  if (!jbdKey()) return { shows: [] as Show[], warnings: ["JamBase API key is not configured."] };
  const cities = jamBaseCitiesForSearch(options.origin, options.radiusMiles);
  const settled = await Promise.allSettled(cities.map((city) => fetchJamBaseCity(city, options.origin, options.radiusMiles, options.days ?? 30)));
  const shows: Show[] = [];
  ...
  return { shows: dedupeShows(shows), warnings };
}
...
function compactArtist(value: string) {
  return normalized(value).split(" ").slice(0, 5).join(" ");
}
...
function key(show: Show) {
  return `${show.date}|${compactArtist(show.artist)}|${compactVenue(show.venue)}`;
}
...
export async function getLiveShows(options?: { postalCode?: string; radiusMiles?: number }): Promise<LiveShowsResponse> {
  ...
  return {
    ...
    providers: {
      Ticketmaster: { configured: tmConfigured, status: !tmConfigured ? "missing" : tmError ? "error" : "ok", rawCount: ticketmaster.length, note: tmError || undefined },
      JamBase: { configured: jbConfigured, status: !jbConfigured ? "missing" : jbError ? "error" : "ok", rawCount: jambase.length, note: jbError || undefined },
