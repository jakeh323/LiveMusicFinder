  ...
  return (
    <main className="page-shell">
      ...
      <section className="content" id="shows">
        <div className="section-head"><div><span className="micro">YOUR WEEKEND</span><h2>Best shows near {areaLabel}</h2></div><div className="data-status"><span className={`live-dot ${dataMode}`}></span><b>{dataMode === "demo" ? "Demo data" : dataMode === "live" ? "Live sources" : "Hybrid live data"}</b><small>{results.length} unique shows · {initialData.rawCandidateCount} raw candidates</small></div></div>

        <div className="provider-strip" aria-label="Live data source health">
          {providerEntries.map(([name, provider]) => <div className={`provider-pill ${provider.status}`} key={name}>
            <span></span><b>{name}</b><strong>{provider.rawCount}</strong>
            <small>{provider.status === "ok" ? "live" : provider.status === "missing" ? "key needed" : "error"}</small>
          </div>)}
        </div>
        {!hasJamBase && <div className="source-alert"><b>Coverage upgrade ready:</b> add a <code>JBD_API_KEY</code> in Vercel to turn on JamBase alongside Ticketmaster. Until then, JamBase will show 0 above.</div>}
