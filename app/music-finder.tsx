...
function actionLabel(show: Show) {
  return show.source === "Ticketmaster" || show.source === "JamBase" ? "Buy Tickets" : "Show Details";
}

function ShowActionLink({ show, compact = false }: { show: Show; compact?: boolean }) {
  if (!show.eventUrl) return null;
  return <a className={compact ? "ticket-link compact" : "ticket-link"} href={show.eventUrl} target="_blank" rel="noopener noreferrer" onClick={(e)=>e.stopPropagation()}>{actionLabel(show)} →</a>;
}

export default function MusicFinder({ initialData, venues }: { initialData: LiveShowsResponse; venues: Venue[] }) {
  const [location, setLocation] = useState(initialData.search.postalCode);
  const [zipError, setZipError] = useState("");
  const [dateFilter, setDateFilter] = useState<DateFilter>("This Weekend");
  const [radius, setRadius] = useState(initialData.search.radiusMiles);
  ...
  const areaLabel = `${initialData.search.city}${initialData.search.state ? `, ${initialData.search.state}` : ""} ${initialData.search.postalCode}`;
  const providerEntries = Object.entries(initialData.providers);
  const hasJamBase = initialData.providers.JamBase?.status === "ok";
  const dateLabel = dateFilter === "All" ? "loaded window" : dateFilter.toLowerCase();
  const providerFilteredCounts = useMemo(() => Object.fromEntries(providerEntries.map(([name]) => [name, results.filter((show) => {
    if (name === "Venue Direct") return show.source === "Venue" || (show.verifiedBy || []).includes("Venue Direct");
    if (name === "Local Calendars") return show.source === "Independent" || show.source === "Community";
    return (show.verifiedBy || [show.source]).includes(name);
  }).length])), [providerEntries, results]);
  ...
  function runSearch() {
    ...
    setZipError("");
    window.location.assign(`/?zip=${encodeURIComponent(zip)}&radius=${radius}`);
  }
  ...
  return (
    <main className="page-shell">
      ...
      <section className="hero">
        <div className="eyebrow"><span></span> LIVE MUSIC, RANKED</div>
        ...
        <div className="search-panel">
          <label className="field location-field"><span>ZIP CODE</span><div><MapPin size={18}/><input value={location} onChange={(e)=>{ setLocation(e.target.value.replace(/\D/g, "").slice(0, 5)); setZipError(""); }} onKeyDown={(e)=>{ if (e.key === "Enter") runSearch(); }} inputMode="numeric" pattern="[0-9]{5}" placeholder="75201" aria-label="ZIP code" /></div>{zipError && <small className="field-error">{zipError}</small>}</label>
          ...
          <label className="field"><span>DISTANCE</span><div><LocateFixed size={18}/><select value={radius} onChange={(e)=>setRadius(Number(e.target.value))}><option value="5">5 miles</option><option value="10">10 miles</option><option value="25">25 miles</option><option value="50">50 miles</option><option value="75">75 miles</option><option value="100">100 miles</option></select><ChevronDown size={16}/></div></label>
          <button className="search-btn" type="button" onClick={runSearch}><Search size={20}/> Find Shows</button>
        </div>
      </section>

      <section className="content" id="shows">
        <div className="section-head"><div><span className="micro">YOUR WEEKEND</span><h2>Best shows near {areaLabel}</h2></div><div className="data-status"><span className={`live-dot ${dataMode}`}></span><b>{dataMode === "demo" ? "Demo data" : dataMode === "live" ? "Live sources" : "Hybrid live data"}</b><small>{results.length} shows for {dateLabel} · {initialData.rawCandidateCount} accepted candidates in the loaded window</small></div></div>

        <div className="provider-strip" aria-label="Live data source health">
          {providerEntries.map(([name, provider]) => <div className={`provider-pill ${provider.status}`} key={name} title={provider.note || ""}>
            <span></span><b>{name}</b><strong>{providerFilteredCounts[name] || 0}</strong>
            <small>{providerFilteredCounts[name] || 0} for {dateLabel} · {provider.rawCount} loaded{provider.rejectedCount ? ` · ${provider.rejectedCount} junk removed` : ""}</small>
            {provider.status !== "ok" && <small className="provider-note">{provider.status === "missing" ? "key needed" : provider.note || (provider.status === "empty" ? "connected, 0 returned" : "error")}</small>}
          </div>)}
        </div>
        {!hasJamBase && <div className="source-alert"><b>JamBase diagnostic:</b> {initialData.providers.JamBase?.status === "missing" ? <>add a <code>JBD_API_KEY</code> in Vercel to turn it on.</> : <>{initialData.providers.JamBase?.note || "JamBase is connected but returned no nearby events."}</>}</div>}
        ...
        {best ? <article className="featured" onClick={()=>setSelected(best)}>
          ...
          <div className="featured-side"><span>{best.distanceMiles} mi away</span><strong>{best.priceText}</strong><ShowActionLink show={best}/><button type="button">See why it's #1 →</button></div>
        ...
        <div className="show-grid">
          {results.slice(1).map((show, i)=><article className="show-card" key={show.id} onClick={()=>setSelected(show)}>
            ...
            <div className="card-bottom"><span>{show.genre} · {show.venueSize}</span><strong>{show.priceText}</strong></div><ShowActionLink show={show} compact/>
          </article>)}
        </div>
      </section>

      <section className="method" id="venues">
        <span className="micro">PHASE 2 COVERAGE</span><h2>{venues.length} DFW venues tracked</h2>
        <p>We combine venue-direct calendars, city/tourism music calendars, and major ticketing feeds so local bars and independent rooms can appear next to theaters, arenas, and stadiums.</p>
        <div className="show-grid">{venues.slice(0, showAllVenues ? venues.length : 18).map((venue)=><article className="show-card" key={venue.id}><h3>{venue.name}</h3><p className="venue">{venue.city}, {venue.state}</p><div className="card-bottom"><span>{venue.venueSize}</span><strong>{venue.collection === "venue-page" ? "Venue direct" : venue.collection === "ticketmaster" ? "Ticketing feed" : "Registry"}</strong></div></article>)}</div>
        {venues.length > 18 && <button className="filter venue-toggle" onClick={()=>setShowAllVenues(v=>!v)}>{showAllVenues ? "Show fewer venues" : `Show all ${venues.length} venues`}</button>}
      </section>
      ...
      <section className="method" id="how">
        ...
        {hasJamBase && <p className="attribution">Concert coverage includes JamBase Data.</p>}
      </section>
      ...
      {selected && <div className="modal-backdrop" onClick={()=>setSelected(null)}><div className="modal" role="dialog" aria-modal="true" onClick={e=>e.stopPropagation()}>
        ...
        <div className="modal-actions"><ShowActionLink show={selected}/></div><div className="modal-foot"><span><Users size={15}/>{selected.venueSize} venue</span><span><Ticket size={15}/>{selected.priceText}</span><span>Verified by {(selected.verifiedBy || [selected.source]).join(" + ")}</span></div>
      </div></div>}
    </main>
  );
}
