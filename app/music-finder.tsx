  ...
  return (
    <main className="page-shell">
      ...
      {selected && <div className="modal-backdrop" onClick={()=>setSelected(null)}><div className="modal" role="dialog" aria-modal="true" onClick={e=>e.stopPropagation()}>
        ...
        <div className="modal-foot"><span><Users size={15}/>{selected.venueSize} venue</span><span><Ticket size={15}/>{selected.priceText}</span><span>Verified by {(selected.verifiedBy || [selected.source]).join(" + ")}</span></div>
      </div></div>}
    </main>
  );
}
