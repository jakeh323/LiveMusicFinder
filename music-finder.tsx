...
import { isHiddenGem, showScore } from "@/lib/scoring";
...
export default function MusicFinder() {
  ...
  const [shows, setShows] = useState<Show[]>(demoShows);
  const [dataMode, setDataMode] = useState<LiveShowsResponse["mode"]>("demo");
  ...
  useEffect(() => {
    ...
    fetch("/api/shows")
      .then((response) => {
        ...
        return response.json() as Promise<LiveShowsResponse>;
      })
      .then((payload) => {
        if (cancelled) return;
  ...
  return (
    <main className="page-shell">
      ...
      <section className="content" id="shows">
        <div className="section-head"><div><span className="micro">YOUR WEEKEND</span><h2>Best shows near Dallas</h2></div><div className="data-status"><span className={`live-dot ${dataMode}`}></span><b>{loadingLive ? "Checking live sources…" : dataMode === "demo" ? "Demo data" : dataMode === "live" ? "Live sources" : "Hybrid live data"}</b><small>{results.length} shows ranked by ShowScore</small></div></div>
